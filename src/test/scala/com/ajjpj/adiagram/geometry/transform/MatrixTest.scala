...
class MatrixTest extends FunSuite with ShouldMatchers {
  ...
  test("matrix scale") {
    val v = Matrix.scale(3) * (2., 3.)
    v.x should be (6. plusOrMinus(eps))
    v.y should be (9. plusOrMinus(eps))
  }

  test("matrix rotate") {
    val v = Matrix.rotate(Math.PI/2) * (1., 0.)
    v.x should be ( 0. plusOrMinus(eps))
    v.y should be (-1. plusOrMinus(eps))
  }
}

...
data class PointedArrowLineEnd(val arrowLineLength: Double = 30.0, val arrowAngle: Double = Math.PI/6): ALineEnd {
    private val sinA = Math.sin(arrowAngle)

    override fun shortenLengthUnzoomed(style: LineStyle) = style.widthNoZoom / sinA * .75 //TODO refine this based on the actual angle
    ...
    override fun paint(gc: GraphicsContext, p: APoint, angle: Angle, style: LineStyle, t: AffineTransformation, zoom: Zoom) {
        ...
        val tipRaw = t(p) + APoint(angle, style.width(Zoom.Identity)/2 / sinA)

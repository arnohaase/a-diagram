...
class RoundArrowLineEnd(arrowLengthFactor: Double = 6.0, arrowAngle: Angle = Angle(Math.PI/6)) extends RenderableLineEnd {
  ...
  override def shortenLength(style: LineStyle, m2s: Model2Screen) = style.width * 1.5 //style.widthNoZoom * .25 //TODO refine this (based on the actual angle)
  ...
  override def paint(gc: GraphicsContext, p: Vector2, angle: Angle, style: LineStyle, t: Vector2, m2s: Model2Screen) {
    ...
    val tipRaw = p + Vector2.fromAngle(angle, style.width/2)

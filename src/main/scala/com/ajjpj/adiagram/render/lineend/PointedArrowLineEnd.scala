...
class PointedArrowLineEnd     (arrowLengthFactor: Double = 6.0, arrowAngle: Angle = Angle(Math.PI/6)) extends RenderableLineEnd {
  ...
  override def shortenLength(style: LineStyle, m2s: Model2Screen) = style.width / sinA * .75 //TODO refine this based on the actual angle

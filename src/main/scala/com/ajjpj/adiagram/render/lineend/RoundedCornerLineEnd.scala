...
class RoundedCornerLineEnd(cornerFraction: Double = .3) extends RenderableLineEnd {
  ...
  override def shortenLength(style: LineStyle, m2s: Model2Screen) = Length.max(Length.ZERO, cornerRadius(style.width) - m2s.overlapLength)
  ...
  override def paint(gc: GraphicsContext, p: Vector2, angle: Angle, style: LineStyle, t: Vector2, m2s: Model2Screen) {
    ...
    val rect0 = m2s.toScreenCoordinates(p + Vector2.fromAngle(angle.cw90,  style.width/2 - r + m2s.overlapLength), t)
    val rect1 = m2s.toScreenCoordinates(p + Vector2.fromAngle(angle.ccw90, style.width/2 - r + m2s.overlapLength), t)
    val rect2 = m2s.toScreenCoordinates(c + Vector2.fromAngle(angle.ccw90, style.width/2 - r + m2s.overlapLength), t)
    val rect3 = m2s.toScreenCoordinates(c + Vector2.fromAngle(angle.cw90,  style.width/2 - r + m2s.overlapLength), t)

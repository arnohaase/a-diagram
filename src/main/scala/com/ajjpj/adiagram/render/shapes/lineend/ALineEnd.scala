...
trait ALineEnd {
  ...
  def shortenLength(style: LineStyle): Double

  def width(style: LineStyle): Double
   ...
   */
  def paint(gc: GraphicsContext, p: APoint, angle: Angle, style: LineStyle, t: Translation): Unit
}
